  ...
  return (
    <>
        ...
        <DialogContent className="sm:max-w-md">
            ...
            <div className="flex items-center gap-2">
              <div className="grid flex-1 gap-2">
                ...
                <Input
                  ...
                  onKeyDown={async (e) => {
                    if (e.key === "Enter") {
                      const success = await updateProject(editingProject.id, e.currentTarget.value.trim());
                      if (success) {
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button
              ...
              onClick={async () => {
                if (editingProject) {
                  const input = document.getElementById('edit-project-input') as HTMLInputElement;
                  const success = await updateProject(editingProject.id, input.value.trim());
                  if (success) {
                    setEditingProject(null);
                  } else {
                    alert("Failed to update project name");
      ...
      <Dialog open={!!deletingProject} onOpenChange={() => setDeletingProject(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-red-600">Delete Job Permanently</DialogTitle>
          </DialogHeader>
          {deletingProject && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                Are you sure you want to delete "<strong>{deletingProject.name}</strong>" permanently?
              </p>
              <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
                <p className="text-sm text-yellow-800">
                  <strong>Warning:</strong> This action cannot be undone. The project and all its map data will be permanently deleted from your browser's local database.
                </p>
              </div>
            </div>
          )}

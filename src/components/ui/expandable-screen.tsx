...
export function ExpandableScreenContent({
  ...
  const { isExpanded, collapse, layoutId, contentRadius, animationDuration } =
    ...
    useEffect(()=>{
      if(overrideClose) collapse()
    },[overrideClose])

  return (
    <AnimatePresence initial={false}>
      {isExpanded && (
        <div className="fixed inset-0 z-500 flex items-center justify-center p-3 sm:p-2">
          ...
          >
            <motion.div
              ...
              className="relative z-500 w-full m-auto"
            >
              {children}
            </motion.div>
            ...
            {showCloseButton && (
              <motion.button
                onClick={collapse}
                className={`absolute right-56 top-28 z-900 flex h-10 w-10 items-center justify-center transition-colors rounded-full ${
                  closeButtonClassName ||
